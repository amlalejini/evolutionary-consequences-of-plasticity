...
combos.register_var('HITCHHIKER_TYPE')
combos.register_var('HITCHHIKER_MAGNITUDE')
combos.add_val(
    'EVENT_FILE_PREFIX',
    [
        'events-ALL-u0',
...
def main():
    ...
    for condition_dict in combo_list:
        ...
        analysis_commands += f' -set ANALYZE_FILE analysis.cfg'
        analysis_commands += ' -a\n'
        ...
        # Format configuration parameters for the run
        run_params = f'-c avida-{condition_dict["HITCHHIKER_TYPE"]}.cfg'
        run_params += f' -set KABOOM_PROB {condition_dict["HITCHHIKER_MAGNITUDE"]}'
        run_params += f' -set POISON_PENALTY {condition_dict["HITCHHIKER_MAGNITUDE"]}'
