...
combos.register_var('BOTTLENECK')
...
combos.add_exception({"EVENT_FILE_PREFIX": 'events-ALL-u0', 'DISABLE_SENSORS' : '0'})
combos.add_exception({"EVENT_FILE_PREFIX": "events-chg-u100" , "BOTTLENECK": '10', 'DISABLE_SENSORS': '1'})
combos.add_exception({"EVENT_FILE_PREFIX": "events-chg-u100" , "BOTTLENECK": '36', 'DISABLE_SENSORS': '1'})
...
def main():
    ...
    for condition_dict in combo_list:
        ...
        run_params += f' -set ENVIRONMENT_FILE environment-L77_phase-one.cfg'
        run_params +=  ' -set COPY_MUT_PROB 0.0025'
        run_params += f' -set DISABLE_REACTION_SENSORS {condition_dict["DISABLE_SENSORS"]}'
        ...
        analysis_commands += f' -set ANALYZE_FILE avida-analysis-L77.cfg'
        analysis_commands += ' -a\n'
        ...
        if condition_dict['DISABLE_SENSORS'] == '0' and condition_dict["EVENT_FILE_PREFIX"] != "events-ALL-u0":
        ...
        event_file_name = f"{condition_dict['EVENT_FILE_PREFIX']}_phase-two_kp-{condition_dict['BOTTLENECK']}.cfg"

        run_params =  f'-set EVENT_FILE {event_file_name}'
        run_params += f' -set ENVIRONMENT_FILE {env_file_name}'
        run_params +=  ' -set COPY_MUT_PROB 0.0025'
        run_params += f' -set DISABLE_REACTION_SENSORS {condition_dict["DISABLE_SENSORS"]}'
        run_params +=  ' -set REACTION_SENSORS_NEUTRAL 0.0'
        run_params +=  ' -set PHYLOGENY_SNAPSHOT_RES 200000'
