...
```{r}
ggplot(filter(agg_data, task_set=="l77"), aes(x=sensors, y=dom_extra_tasks, color=sensors)) +
  geom_boxplot() +
  scale_x_discrete(name="Sensors", limits=c(TRUE, FALSE), labels=c("T","F")) +
  facet_grid(task_set~environment) +
...
```{r}
ggplot(filter(agg_data, task_set=="l77"), aes(x=sensors, y=pop_extra_tasks, color=sensors)) +
  geom_boxplot() +
  scale_x_discrete(name="Sensors", limits=c(TRUE, FALSE), labels=c("T","F")) +
