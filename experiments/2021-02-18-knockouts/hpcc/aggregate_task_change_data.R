...
colnames(df_out) = c('x', 'seed', 'environment', 'sensors', 'task_changes_per_mut')

for(seed_dir in list.dirs(base_dir, recursive = F)){
    filename = paste0(seed_dir, '/task_changes_per_mut.csv')
...
write.csv(df_out, 'task_changes_per_mut_summary.csv')
