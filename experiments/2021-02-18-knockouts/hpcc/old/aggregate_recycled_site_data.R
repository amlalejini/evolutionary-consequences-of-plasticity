...
colnames(df_out) = c('seed', 'environment', 'sensors', 'recycled_frac')
    ...
    for(seed_dir in list.dirs(treatment_dir, recursive = F)){
        ...
        if(!file.exists(filename)){
            ...
            num = sum(df_tmp[
                ...
                df_tmp$high_level_functionality %in% c('Even Recycled Odd', 'Odd Recycled Even')
                    ,]$high_level_length)
            denom = sum(df_tmp[df_tmp$is_function_change,]$high_level_length)
            recycled_frac = num / denom
            df_tmp$recycled_frac = recycled_frac
            cat(df_tmp$seed[1], ' ')
...
write.csv(df_out, 'recycled_frac_summary.csv')
