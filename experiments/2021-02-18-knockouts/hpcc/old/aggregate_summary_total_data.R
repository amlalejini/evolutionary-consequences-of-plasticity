...
scrape_dir = function(dir_name){
    filename = paste0(dir_name, '/summary_total.csv')
...
for(dir_name in dir_vec[1:length(dir_vec)]){
  ...
  data_seed$high_level_functionality = as.character(data_seed$high_level_functionality)
...
# Save the data!
data_aggregate$high_level_functionality = as.character(data_aggregate$high_level_functionality)
write.csv(data_aggregate, paste0(output_path, 'summary_total.csv'))
